...
var cornerRadius = 20 * (width / 1220);                // why choose this radius
var midx = width / 2;
var midy = height / 2;
...
//Adding initial offset to drawMovingSpace() function
...
// check if mousepointer is in between x, y and x2, y2
function checkLine(x, y, x2, y2) {
    if (mousex < Math.min(x, x2) - 5.0 ||
            mousex > Math.max(x, x2) + 5.0 ||
            mousey < Math.min(y, y2) - 5.0 ||
            mousey > Math.max(y , y2) + 5.0) {
        return false;
    }
...
// returns true if distance between x,y and line x1,y1 and x2,y2 is less then 5
function checkLinePoint(x, y, x1, y1, x2, y2) {
    if (x <= Math.min(x1, x2) - 5.0 ||
            x >= Math.max(x1, x2) + 5.0 ||
            y <= Math.min(y1, y2) - 5.0 ||
            y >= Math.max(y1, y2) + 5.0) {
        return false;
    }
    var a = y2 - y1;
    var b = x1 - x2;
...
// returns true if distance between x,y and line x1,y1 and x2,y2 is less then 5
function checkCirclePoint(xc, yc, x1, y1, x2, y2, r) {
    if(x1 == x2) {
        return false;
    } else {
        var m = (y2 - y1) / (x2 - x1);
...
// if not working then use mod
function calculateCircleAngle(x1, y1, x2, y2, x3, y3) {
